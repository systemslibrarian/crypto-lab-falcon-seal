 ...
 *
 *  - text over a surface axe declines to resolve. This page is built out of
 *    them. `html, body` paints TWO `radial-gradient` washes over the whole
 *    canvas, and almost every inset surface on the page is a `color-mix()`
 *    over whatever is behind it: `.output` (which is where every generated
 *    key, signature, forgery verdict and trapdoor table lands) is
 *    `color-mix(in srgb, var(--text) 4%, transparent)`, and so are `.quiz`,
 *    `.attempt-row`, `.attempt-bar`, `.c-strip`, `.histo`, `.attack-spark`,
 *    `.vsize-strip` and the `.lattice-bg` behind the SVG figures. axe files all
 *    of those under `incomplete`, which means a violations-only gate measured
 *    the contrast of almost nothing this lab actually generates.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. Here the fades are all in
 *    `@media (prefers-reduced-motion: no-preference)` blocks (`rise` on every
 *    panel, `attempt-in` on every sampling row, `tour-in` on the tour card),
 *    so with the preference set they must not apply at all — which is exactly
 *    what `expectNotBlank` in the gate is there to prove.
 *
 ...
 *    boxes here are `.table-wrap`, `.key-size-table` and `.viz` (each
 *    `overflow-x: auto` around content with a `min-width` of 400-700px, so at
 *    380px most of every comparison table and the whole right half of the
 *    lattice figure is clipped away), `.output` (also `overflow-x: auto`, and
 *    it holds unbroken 1000-character polynomial dumps), and `body` itself.
 *    Content scrolled past the client box is not dimmed or partly drawn — it is
 *    absent from the frame, and asking what colour it sits on has no answer.
 *    Its rect is still outside every ancestor's box, so the ancestor walk would
 ...
 *    is load-bearing here rather than defensive: the lattice figures are real
 *    SVG, and every `.svg-label` sits on the `.lattice-bg` `<rect>` drawn
 *    before it, not on any ancestor. An ancestor-only walk would composite
 *    those labels onto the `<svg>`'s own transparent background and report a
 *    ratio nothing on screen has. So for SVG text, any earlier sibling shape
 *    whose box contains the text box is composited first, and SVG ink is read
 *    from `fill` rather than from `color`.
 *
 ...
 * rule. This page's only generated content is the UA `::marker` on the
 * `.paste-details` disclosure triangle and on the reference lists.
 *
 * SECOND, WCAG 1.4.11: control boundaries and meaningful graphics. This lab is
 * dense with the latter — a challenge-polynomial strip, four bar charts, two
 * meters, a histogram, a sparkline and two SVG lattice figures — and none of it
 * owns a text node, so nothing here or in axe can reach it. Those were measured
 * by hand from screenshot pixels.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * surfaces with `color-mix()` in both `srgb` (every `.output`, `.quiz`,
     * `.c-strip` and chart track) and `oklab` (the hero's "why it matters"
     * wash), and Chromium reports those to `getComputedStyle` unchanged rather
     * than converting them to sRGB. A
     ...
     *
     * Judging every gradient at its worst *stop* assumes that stop covers the
     * text wherever the text sits. That is right for a gradient that spans its
     * element, and wrong here: `html, body` paint two low-alpha radial washes
     * that fade to `transparent` by 42-45% — decorative glows at 10%/20% and
     * 90%/0% of the canvas. Judging the footer, several page-heights below, by
     * the corner colour invents a backdrop the footer text never sits on. So
     * each gradient is *sampled* at the text's real location instead: linear by
     * projecting onto the gradient line, radial by distance from the centre
     * over the farthest-corner radius.
     ...
     *
     * A run of Panel 3 renders a 512-cell challenge strip and a rejection-
     * sampling list, Panel 7 renders five polynomial tables, and every one of
     * those cells walks the same handful of ancestors. Without the caches the
     * pass re-reads the same computed styles and rects tens of thousands of
     * times. Nothing mutates the DOM during the pass, so the cached
     ...
     * `aria-hidden` glyph. This lab uses neither idiom today — its skip links
     * park off-screen instead, which the document-space guard below handles —
     * so the check is currently silent here and is kept because the recipe is
     * one `.sr-only` class away from arriving. Deliberately narrow: only a
     * ZERO-area clip qualifies, so a real partial clip is still measured.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page's disclosure is the `.paste-details`
      // paste-a-signature panel, whose open state is a real state the gate
      // scans explicitly by clicking its summary.
      ...
      // `top: -3rem` and the lab's own `.skip-link` at `left: -9999px`, each
      // sliding into view only on focus. Measuring the parked copy invents a failure
     ...
     * stricter than axe on decorative content. Honour the same boundary. On
     * this page that covers the shared header's two icon `<svg>`s, which carry
     * shapes and no characters, so the guard is silent here.
     */
     ...
     *
     * This is not hypothetical on a page that builds its SVG from template
     * literals, as this one does: interpolating a string ARRAY into a template
